...
class User(Base):
    ...
    # Настройки триггеров (в процентах)
    trigger_threshold_positive = Column(Numeric, default=5.0)  # Порог для роста (по умолчанию +5%)
    trigger_threshold_negative = Column(Numeric, default=-5.0)  # Порог для падения (по умолчанию -5%)
    triggers_enabled = Column(Boolean, default=True)  # Включены ли триггеры
